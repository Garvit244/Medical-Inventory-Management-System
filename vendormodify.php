...
<table class="inventory">
				<thead>
					<tr>
						<th colspan="5"><span>Vendor Details</span></th>
					</tr>
				</thead>
				<tbody>
					<tr>
						...
						<td COLSPAN="4"><input type="text" name="medname"></td>
					</tr><tr>
						<td><span>Medicine Batchno:</span></td>
						<td COLSPAN="4"><input type="text" name="medbatchno"></td>
					</tr>
				</tbody>
</table>
...
<?php
	...
	{
		$area = $_POST['area'];
		$query = "UPDATE vendors SET Area = '$area' WHERE ID = $id";
	}
	if (isset ($query))
	{
		$sql = mysql_query($query,$con);
	...
	if($new)
	{
		if(isset($_POST['id']))
		{
			$query = "SELECT ID,Name,Area,Sex,Medname,MedBatchno,Date FROM vendors WHERE ID=".$_POST['id'].";";
		}
		if (isset ($query))
		{
			$sql = mysql_query($query,$con);
		...
		if (isset($sql))
	{
		while($row = mysql_fetch_array($sql))
		{
			echo ("<br /><br /><br /><br /><aside><h1>Details Updated Sucessfully</h1></aside>");
				...
				<thead><tr>
						<th COLSPAN=5><span>Modified Vendor Details</span></th></tr>
				</thead>
				<tbody>");
				$new = $row['ID'];
				echo("<tr>

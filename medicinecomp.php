...
<body style="background-color:skyblue">
	<hr />
	<center> <h2> Medicine Buy From Company Details </h2> </center>	
	<hr />
	<center>
	...
	<table>
	<tr>
	<td><br />Company ID. &nbsp &nbsp &nbsp </td>
	<td><br /><input type="name" name="cid" /><font color="RED" SIZE="2">*</font>&nbsp &nbsp &nbsp &nbsp &nbsp &nbsp </td>
	</tr>
	<tr>
	<td><br />Name &nbsp &nbsp &nbsp </td>
	<td><br /><input type="name" name="cname" /><font color="RED" SIZE="2">*</font>&nbsp &nbsp &nbsp &nbsp &nbsp &nbsp </td>
	</tr>
	<tr>
	...
	if ( isset ($_POST['sid']))
	{
		...
		if (!empty($row))
		{
			...
			if (isset ($query))
				$sql = mysql_query($query,$con);
	if (isset($_POST['cid']) && isset($_POST['cname']) && isset($_POST['area']))
		$query = "INSERT INTO company (ID,Name,Location) VALUES ('$_POST[cid]','$_POST[cname]','$_POST[area]')";
	if (isset ($query))
		$sql = mysql_query($query,$con);
	if (isset ($sql))
	if (!$sql)
	...
	if (!$sql)
	{
		...
		if (isset($_POST['cid']) && isset($_POST['name']) && isset($_POST['batchno']) )
			$query = "INSERT INTO te(ID,Medname,MedBatchno) VALUES ( '".$_POST['cid']."','".$_POST['name']."','".$_POST['batchno']."')";
		if (isset ($query))
			$sql = mysql_query($query,$con);
		if (isset ($sql))
		if (!$sql)
		...
		if (isset($_POST['cid']) && isset($_POST['name']) && isset($_POST['batchno']) && isset($_POST['qty']) && isset($_POST['sid']) && isset($_POST['date']) )
			$query = "INSERT INTO supplied(XID,Medname,MedBatchno,Qty,SSID,Date) VALUES( '".$_POST['cid']."','".$_POST['name']."','".$_POST['batchno']."','".$_POST['qty']."','".$_POST['sid']."','".$_POST['date']."')";
		if (isset ($query))
			$sql = mysql_query($query,$con);
		if (isset ($sql))
		if (!$sql)
